...
public class ConcurrentTest {
	...
	public static void main(String[] args) throws Exception {
			ApplicationContext ctx = new ClassPathXmlApplicationContext("concurrent-app-context.xml");
	...
	public void run() throws Exception {
		...
		for (int i=0; i < 60; i++) {
			long shares = ((long)((new Random().nextDouble()*4000)+1));	//just to vary the numbers of shares
			String trade = "[" + (i+1) + "]: " + "BUY " + shares + " shares of AAPL";
			System.out.println("Placing Trade    " + trade);
			jms.convertAndSend(trade);	//send message to the queue
		}
